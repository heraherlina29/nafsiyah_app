...
if (!empty($search_query)) {
    // Pencarian diperluas ke kolom no_wa
    $where_clauses[] = "(username LIKE ? OR no_wa LIKE ?)";
    $params[] = "%$search_query%";
    $params[] = "%$search_query%";
}
...
// Tambahkan no_wa ke SELECT
$data_sql = "SELECT id, username, no_wa, created_at " . $base_sql . " ORDER BY created_at DESC LIMIT ? OFFSET ?";
        ...
        <form action="admin.php" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            <div class="md:col-span-10">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" placeholder="Cari berdasarkan username atau WhatsApp..." 
                                        ...
                                        <tr class="hover:bg-slate-50/50 transition-colors dark:hover:bg-dark-surface2/50 group">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-[10px] text-slate-400">#<?= $admin['id'] ?> - Super Admin</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <button type="button" 
                                                            class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-500 hover:bg-primary-50 hover:text-primary-600 edit-btn dark:bg-dark-surface2 dark:border-slate-700 dark:text-slate-400"
                                                            data-id="<?= $admin['id'] ?>" 
                                                            data-username="<?= htmlspecialchars($admin['username']) ?>"
                                                            data-no_wa="<?= htmlspecialchars($admin['no_wa']) ?>"
                                                            title="Edit">
                                                    ...
                                                    <?php if ($admin['id'] != $_SESSION['user_id']): ?>
                                                            <button type="button" 
                                                                    class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-500 hover:bg-rose-50 hover:text-rose-600 hapus-btn dark:bg-dark-surface2 dark:border-slate-700 dark:text-slate-400"
                                                                    data-id="<?= $admin['id'] ?>" 
                                                                    data-username="<?= htmlspecialchars($admin['username']) ?>"
                                                                    title="Hapus">
                                                    ...
                                                    <?php else: ?>
                                                            <span class="w-8 h-8 flex items-center justify-center text-slate-300" title="Akun Anda">
                    ...
                    <div class="flex gap-2">
                        <?php for ($i = 1; $i <= $total_halaman; $i++): ?>
                                    <a href="?page=<?= $i ?>&search=<?= urlencode($search_query) ?>" 
                                       class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all <?= $i == $halaman_aktif ? 'bg-primary-600 text-white' : 'bg-white text-slate-500 border dark:bg-dark-surface dark:border-slate-600' ?>">
                                        <?= $i ?>
                                    </a>
                        <?php endfor; ?>
                    </div>
...
<div id="dataModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl transform scale-95 transition-transform duration-300 overflow-hidden dark:bg-dark-surface border border-slate-100 dark:border-slate-700" id="modalContent">
        ...
        <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-dark-surface2/50">
            ...
            <button id="closeModalBtn" class="w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 flex items-center justify-center hover:bg-rose-50 hover:text-rose-500 transition-all dark:bg-dark-surface2 dark:border-slate-600">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="username" id="username" required 
                           class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 dark:bg-dark-surface2 dark:border-slate-700 dark:text-white"
                           placeholder="username">
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-xs font-bold text-slate-500 uppercase tracking-wider dark:text-slate-400">WhatsApp</label>
                <div class="relative">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" id="password" 
                           class="w-full pl-11 pr-10 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 dark:bg-dark-surface2 dark:border-slate-700 dark:text-white"
                           placeholder="••••••••">
                ...
                <p id="passwordHint" class="text-[10px] text-slate-400 italic mt-1 dark:text-slate-500">* Wajib untuk admin baru</p>
            </div>
            
            <div class="pt-4 flex gap-3">
                <button type="button" id="cancelModalBtn" class="flex-1 px-4 py-3 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-xl dark:bg-dark-surface2 dark:border-slate-600 dark:text-slate-300">
                    Batal
                </button>
                <button type="submit" class="flex-[2] px-4 py-3 bg-primary-600 text-white text-sm font-bold rounded-xl hover:bg-primary-700 shadow-lg">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('no_wa').value = d.no_wa; // Isi data WA ke modal
    ...
    // Proses Simpan dan Hapus tetap menggunakan logika Fetch API yang sudah ada
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            Swal.fire('Berhasil!', 'Data admin dihapus.', 'success').then(() => location.reload());
                        } else { throw new Error(data.message); }
                    })
                    .catch(err => Swal.fire('Gagal!', err.message, 'error'));
                }
